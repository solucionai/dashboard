...
#     # Se o botão "Home" for clicado ou se não houver cliques, mostrar a página Home como padrão
#     if n_home or not (n_home or n_leads or n_analise or n_atendentes):
...
#         ])
#     elif n_analise:
#         # 5. Contratos Fechados por Tempo (Movido para Análise Detalhada de Leads)
#         contratos_by_date = df[df['CONTRATO ASSINADO'] == 1].groupby(df['created_at'].dt.date).size()
#         fig_contratos_tempo = px.line(x=contratos_by_date.index, y=contratos_by_date.values, title="Contratos Fechados por Período",
#                                       labels={'x': 'Data', 'y': 'Número de Contratos Fechados'})

#         return html.Div([
#             dcc.Graph(figure=fig_contratos_tempo)  # Gráfico 5: Contratos fechados por tempo (Análise Detalhada)
#         ])
#     elif n_atendentes:
#         return html.Div("Aqui estão os dados dos Atendentes.")
#     else:
#         return html.Div("Selecione uma opção na barra lateral.")
...
# # Inicializar o aplicativo Dash com suppress_callback_exceptions=True
...
#                 ),
#                 dcc.Graph(id='tempo-por-lead-wpp-graph'),  # Gráfico de tempo na etapa por lead com número de WhatsApp
#                 dcc.Graph(figure=fig_total_leads),  # Gráfico 1: Total de Leads
#                 dcc.Graph(figure=fig_leads_problema),  # Gráfico 2: Leads por Tipo de Problema
#                 dcc.Graph(figure=fig_leads_ddd),  # Gráfico 3: Leads por Região ou DDD
#                 dcc.Graph(figure=fig_fluxo_completo),  # Gráfico 4: Leads que completaram o fluxo
...
# # Callback para o gráfico de tempo na etapa por número de WhatsApp com filtro de problema
# @app.callback(
#     Output('tempo-por-lead-wpp-graph', 'figure'),
#     [Input('problema-dropdown', 'value')]
# )
# def update_tempo_por_lead_graph(selected_problema):
#     # Filtrar os dados pelo problema selecionado
#     df_filtered = df[df['Tempo_na_Etapa'] <= 40]  # Excluir valores maiores que 40 minutos
#     df_filtered = df_filtered[df_filtered['Tempo_na_Etapa'] > 0]  # Excluir valores 0
...
#                 dcc.Graph(figure=fig_leads_ddd),  # Gráfico 3: Leads por Região ou DDD
#                 dcc.Graph(figure=fig_fluxo_completo),  # Gráfico 4: Leads que completaram o fluxo
#                 dcc.Graph(id='tempo-por-lead-wpp-graph')  # Último gráfico: Tempo na Etapa por Lead
#             ])
#         elif button_id == "analise-leads-button":
#             # 5. Contratos Fechados por Tempo
#             contratos_by_date = df[df['CONTRATO ASSINADO'] == 1].groupby(df['created_at'].dt.date).size()
...
# # Callback para o contador e gráfico de tempo na etapa por número de WhatsApp
# @app.callback(
#     [Output('leads-info', 'children'),
#      Output('tempo-por-lead-wpp-graph', 'figure')],
#     [Input('problema-dropdown', 'value')]
# )
# def update_leads_info_and_tempo_graph(selected_problema):
#     # Filtrar os dados pelo problema selecionado
#     df_filtered = df[df['Tempo_na_Etapa'] <= 40]  # Excluir valores maiores que 40 minutos
#     df_filtered = df_filtered[df_filtered['Tempo_na_Etapa'] > 0]  # Excluir valores 0
...
#     return leads_info, fig_tempo_por_lead_wpp
...
#                 # Formatando as informações para exibir corretamente
#                 leads_info = [
#                     html.H4(f"Leads Captados no Problema Selecionado: {leads_captados}"),
#                     html.H4(f"Total de Leads Captados: {leads_captados_total}"),
#                     html.H4(f"Proporção de Leads no Problema Selecionado: {proporcao_responderam:.2f}%")
...
#                 return html.Div([
#                     dcc.Graph(figure=fig_total_leads),  # Gráfico 1: Total de Leads
#                     html.Div(leads_info, style={'text-align': 'center', 'margin': '20px 0'}),  # Contador de leads captados
#                     dcc.Graph(figure=fig_leads_problema),  # Gráfico 2: Leads por Tipo de Problema
#                     dcc.Graph(figure=fig_leads_ddd),  # Gráfico 3: Leads por Região ou DDD
#                     dcc.Graph(figure=fig_fluxo_completo),  # Gráfico 4: Leads que completaram o fluxo
#                 ])
...
#                 # Adicionando o dropdown para selecionar o problema
...
#                 # Filtrar dados para calcular os leads captados por problema
#                 df_filtered_by_problem = df[df['PROBLEMA'] == dropdown_problema.value]  # Filtra os dados pelo problema selecionado
